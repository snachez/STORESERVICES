...
namespace STORESERVICES.API.SHOPPINGCART.DAO.MethodsConnection
{
    public class DatabaseHub : Connection, IDatabaseHub
    {
        ...
        public async Task<List<T>> QueryAsync<T>(string storedProcedureName, DynamicParameters parameters, ICarritoContexto carritoContexto)
        {
            using (var connection = LiveConnection(carritoContexto))
            {
                try
                {
        ...
        public async Task<long> ExecuteAsync<TModel>(string storedProcedureName, TModel model, ICarritoContexto carritoContexto)
        {
        ...
        public async Task<T> QueryParameterAsync<T>(string storedProcedureName, DynamicParameters parameters, ICarritoContexto carritoContexto)
        {
            using (var connection = LiveConnection(carritoContexto))
            {
                try
        ...
        public async Task<long> ExecuteAsync(string storedProcedureName, DynamicParameters parameters, ICarritoContexto carritoContexto)
        {
            using (var connection = LiveConnection(carritoContexto))
            {
                try
