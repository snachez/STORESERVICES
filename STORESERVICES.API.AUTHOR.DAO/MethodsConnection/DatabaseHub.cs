...
namespace STORESERVICES.API.AUTHOR.DAO.MethodsConnection
{
    public class DatabaseHub : Connection, IDatabaseHub
    {
        ...
        public async Task<List<T>> QueryAsync<T>(string storedProcedureName, IContextoAutor contextoAutor)
        {
            using (var connection = LiveConnection(contextoAutor))
            {
                try
                {
        ...
        public async Task<long> ExecuteAsync<TModel>(string storedProcedureName, TModel model, IContextoAutor contextoAutor)
        {
        ...
        public async Task<T> QueryParameterAsync<T>(string storedProcedureName, IContextoAutor contextoAutor)
        {
            using (var connection = LiveConnection(contextoAutor))
            {
                try
        ...
        public async Task<long> ExecuteAsync(string storedProcedureName, DynamicParameters parameters, IContextoAutor contextoAutor)
        {
            using (var connection = LiveConnection(contextoAutor))
            {
                try
